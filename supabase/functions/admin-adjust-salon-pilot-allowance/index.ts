...
// FaceTune WA-8 — adjust a Salon Pilot allowance.
//
// The second privileged admin mutation, on the WA-7 template (Shared
// Contract §58):
//
//     Web Admin (Flutter Web)
//         ↓  Authorization: Bearer <the admin's own JWT>
//         ↓  { entitlementId, amount, reason, idempotencyKey, expectedVersion? }
//     this function
//         ↓  requireAdmin(): auth.getUser() + current_user_is_admin(), AS THE CALLER
//         ↓  parse + validate the request shape (400 on a client defect)
//         ↓  rpc admin_adjust_salon_pilot_allowance(...), AS THE CALLER
//         ↓     is_admin(auth.uid()) again · per-account lock · rules ·
//         ↓     INSERT entitlement_allowance_adjustments (its trigger moves the
//         ↓     total under the account lock) + INSERT audit event
...
Deno.serve(async (request) => {
  ...
  if (!admin.ok) {
    console.error(
      `[admin-adjust-salon-pilot-allowance] refused code=${admin.errorCode} correlation=${correlationId}`,
    );
  ...
  const action = parsed.ok
    ? adjustmentActionFor(parsed.value.amount)
    : "increase_allowance";
  if (!parsed.ok) {
    console.error(
      `[admin-adjust-salon-pilot-allowance] invalid_request field=${parsed.field} admin=${admin.userId} correlation=${correlationId}`,
    );
  ...
  try {
    ...
    if (error) {
      ...
      if (error.code === "22023") {
        console.error(
          `[admin-adjust-salon-pilot-allowance] invalid_request source=db admin=${admin.userId} correlation=${correlationId}`,
        );
        return jsonResponse(
          invalidRequestBody(
            {
      ...
      console.error(
        `[admin-adjust-salon-pilot-allowance] rpc_failed code=${
          error.code ?? "unknown"
    ...
    console.error(
      `[admin-adjust-salon-pilot-allowance] rpc_threw type=${
  ...
  const entitlementId = typeof (response.body as Record<string, unknown>)
    ...
    : "none";
  console.log(
    `[admin-adjust-salon-pilot-allowance] action=${action} outcome=${response.outcome} admin=${admin.userId} entitlement=${intent.entitlementId} amount=${intent.amount} result_entitlement=${entitlementId} correlation=${correlationId} latency_ms=${
      Date.now() - startedAt
    }`,
  );
