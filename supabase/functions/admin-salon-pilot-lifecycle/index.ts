...
// reactivate, revoke. One bounded domain endpoint (Web Admin SOT §49) that
// dispatches to the two database writers by action, on the WA-7 template
// (Shared Contract §58):
//
//     Web Admin (Flutter Web)
//         ↓  Authorization: Bearer <the admin's own JWT>
//         ↓  { action, entitlementId, reason, idempotencyKey, expectedVersion?, newExpiresAt? }
//     this function
//         ↓  requireAdmin(): auth.getUser() + current_user_is_admin(), AS THE CALLER
//         ↓  parse + validate the request shape (400 on a client defect)
//         ↓  rpc admin_extend_salon_pilot_expiration | admin_set_salon_pilot_lifecycle,
//         ↓     AS THE CALLER: is_admin(auth.uid()) again · per-account lock ·
//         ↓     transition rules · one status/expiry UPDATE + INSERT audit event
...
Deno.serve(async (request) => {
  ...
  if (!admin.ok) {
    console.error(
      `[admin-salon-pilot-lifecycle] refused code=${admin.errorCode} correlation=${correlationId}`,
    );
  ...
  const parsed = parseLifecycleRequest(body);
  const action = parsed.ok
    ? parsed.value.action
    : parsed.action ?? "suspend_entitlement";
  if (!parsed.ok) {
    console.error(
      `[admin-salon-pilot-lifecycle] invalid_request field=${parsed.field} admin=${admin.userId} correlation=${correlationId}`,
    );
    return jsonResponse(invalidRequestBody(parsed, action), 400, correlationId);
  }
  ...
  try {
    ...
    const { data, error } = intent.action === "extend_expiration"
      ? await client.rpc("admin_extend_salon_pilot_expiration", {
        p_entitlement_id: intent.entitlementId,
        p_new_expires_at: intent.newExpiresAt,
      ...
      : await client.rpc("admin_set_salon_pilot_lifecycle", {
        p_entitlement_id: intent.entitlementId,
    ...
    if (error) {
      ...
      if (error.code === "22023") {
        console.error(
          `[admin-salon-pilot-lifecycle] invalid_request source=db admin=${admin.userId} correlation=${correlationId}`,
        );
        return jsonResponse(
          invalidRequestBody(
            {
      ...
      console.error(
        `[admin-salon-pilot-lifecycle] rpc_failed code=${
          error.code ?? "unknown"
    ...
    console.error(
      `[admin-salon-pilot-lifecycle] rpc_threw type=${
  ...
  const response = mutationResponse(result, action);
  const entitlementId = typeof (response.body as Record<string, unknown>)
    ...
    : "none";
  console.log(
    `[admin-salon-pilot-lifecycle] action=${action} outcome=${response.outcome} admin=${admin.userId} entitlement=${intent.entitlementId} result_entitlement=${entitlementId} correlation=${correlationId} latency_ms=${
      Date.now() - startedAt
    }`,
  );
