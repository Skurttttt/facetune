...
import { isUnchanged, requestGeminiGuideline } from "./gemini_client.ts";
...
Deno.serve(async (request: Request) => {
  ...
  let metricStepId: string | null = null;
  ...
  try {
    ...
    metricSessionId = context.tutorialSessionId;
    metricStepId = context.stepId;
    ...
    if (
      step.status === "ready" && typeof step.guideline_storage_path === "string"
    ) {
      console.log(
      ...
      await recordAiOperationMetric(telemetryClient, telemetryEventId, userId, {
        operationKind: "tutorial_step",
        outcome: "duplicate",
        sourceMode: context.sourceMode,
        tutorialSessionId: context.tutorialSessionId,
        tutorialStepId: context.stepId,
      });
      return jsonResponse({
        step: {
          id: context.stepId,
    ...
    if (step.status === "generating") {
      ...
      const heldFor = Number.isNaN(startedAt)
        ? Infinity
        : Date.now() - startedAt;
    ...
    const attempt = (typeof step.generation_attempt === "number"
    ...
    await recordAiOperationMetric(telemetryClient, telemetryEventId, userId, {
      ...
      modelName: TUTORIAL_GUIDELINE_MODEL,
      promptVersion: TUTORIAL_GUIDELINE_PROMPT_VERSION,
      usage: usageForMetric(usageSink),
      outputImages: 1,
      outputImageResolution: TUTORIAL_OUTPUT_RESOLUTION,
    });
    return jsonResponse({
      step: {
        id: context.stepId,
    ...
    if (telemetryClient && metricUserId && metricSessionId && metricStepId) {
