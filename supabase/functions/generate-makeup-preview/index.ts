...
function requiredEnvironment(name: string): string {
  ...
  if (!value) {
    ...
    throw new FunctionFailure(500, "server_configuration", "The image service is not configured.");
  }
  return value;
}
...
function recommendationId(value: unknown): string {
  if (typeof value !== "object" || value === null || Array.isArray(value)) {
    throw new FunctionFailure(400, "invalid_request", "A valid preview request is required.");
  }
  const id = (value as Record<string, unknown>).recommendationId;
  if (typeof id !== "string" || !uuidPattern.test(id)) {
    throw new FunctionFailure(400, "invalid_recommendation_id", "A valid recommendation ID is required.");
  }
  return id;
}
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    if (!authorization?.toLowerCase().startsWith("bearer ")) {
      throw new FunctionFailure(401, "authentication_required", "Sign in before generating a preview.");
    }
    ...
    const { data: recommendation, error: recommendationError } = await userClient
    ...
    if (!["image/jpeg", "image/png", "image/webp"].includes(originalMimeType)) {
      throw new FunctionFailure(422, "invalid_original_type", "The original selfie type is unsupported.");
    }
    console.log(
    ...
    if (imagesAreIdentical(originalBytes, generated.bytes)) {
      throw new FunctionFailure(502, "unchanged_generated_image", "The image service did not apply the makeup plan.", true);
    }
    const extension = extensionFor(generated.mimeType);
    const paddedNumber = generationNumber.toString().padStart(4, "0");
    const candidatePath = `${authData.user.id}/analyses/${analysis.id}/generated/${requestedRecommendationId}/preview_${paddedNumber}.${extension}`;
    if (candidatePath === originalImagePath || candidatePath.includes("/original/")) {
      throw new FunctionFailure(500, "unsafe_storage_path", "A safe preview path could not be created.");
    }
    ...
    uploadedPath = null;
    console.log(`[generate-makeup-preview] Completed model=${model} prompt=${MAKEUP_PREVIEW_PROMPT_VERSION} variation=${generationNumber}`);
    ...
    const failure = error instanceof FunctionFailure
      ? error
      : new FunctionFailure(500, "server_error", "The preview request could not be completed.");
    if (!(error instanceof FunctionFailure)) {
      console.error(`[generate-makeup-preview] Unhandled error type=${error?.constructor?.name ?? "unknown"}`);
    }
