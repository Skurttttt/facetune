...
if [[ -f build/web/main.dart.js ]]; then
  ...
  check "bundle: the read RPCs are the eleven known ones" \
    "$(grep -oE 'admin_(list|get|search|dashboard|salon_pilot_research)_[a-z0-9_]+' build/web/main.dart.js | grep -v '^admin_dashboard_v2_contract_v1$' | sort -u | tr '\n' ',')" \
    "admin_dashboard_metrics,admin_dashboard_v2_metrics,admin_get_audit_event,admin_get_user,admin_list_audit_events,admin_list_entitlement_history,admin_list_entitlements,admin_list_salon_pilot_metrics,admin_list_usage,admin_salon_pilot_research_metrics,admin_search_users,"
else
